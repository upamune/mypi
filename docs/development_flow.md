...
`slopchop` は「人間が今の diff を読む」ための軽い道具として使います。指摘を agent に投げて直すところまで自動で回したい場合は `/review` か `/review-start` に切り替えます。
...
| Package / command | 使うタイミング | 注意 |
| --- | --- | --- |
| `/review` | 現在の diff を correctness、regression、security、missing tests 観点で見たいとき | `mitsupi` 側にも `/review` があるため、解決順に注意 |
